...
MAX_INTERVAL_LAG = INTERVALS_PER_DAY * 20  # 320 intervals = 20 trading days

# Exclude non-tradeable auction bars:
# 09:30 = opening auction (O==C, no free trading)
# 18:00 = closing auction (single-price auction, no free trading)
EXCLUDED_TIMES = {'09:30', '18:00'}

# Generate all 18 original times, then filter to 16 tradeable ones
_ALL_TIMES_18 = []
_h, _m = 9, 30
for _ in range(18):
    _ALL_TIMES_18.append(f"{_h:02d}:{_m:02d}")
    _m += 30
    if _m >= 60:
        _m = 0
        _h += 1

STANDARD_TIMES = [t for t in _ALL_TIMES_18 if t not in EXCLUDED_TIMES]  # 16 intervals: 10:00-17:30
assert len(STANDARD_TIMES) == 16, f"Expected 16 tradeable intervals, got {len(STANDARD_TIMES)}"

HALF_DAY_TIMES = [t for t in STANDARD_TIMES if t <= '12:00']  # 10:00 - 12:00 (5 intervals)
...
def load_all_data():
    ...
    for fpath in csv_files:
        ...
        # Filter out non-tradeable auction bars (09:30 opening, 18:00 closing)
        ...
        n16 = interval_dist.get(16, 0)  # Full day: 16 tradeable intervals
        n5 = interval_dist.get(5, 0)    # Half day: 5 tradeable intervals
        n_other = n_days - n16 - n5
        ...
        summary_rows.append({
            ...
            f'{INTERVALS_PER_DAY}-int Gün': n16,
            '5-int Gün': n5,
            'Diğer Gün': n_other,
            'Mükerrer': n_dupes,
            'Filtrelenen Müzayede Barı': n_filtered,
...
def clean_and_prepare(df):
    ...
    for (ticker, date), count in day_interval_count.items():
        if count == INTERVALS_PER_DAY:  # 16 tradeable intervals
    ...
    if (today_count < INTERVALS_PER_DAY).any():
        df = df[df['date'] != today]
...
def run_periodicity_analysis(df, ret_col='r_oc', label='OC',
    ...
    interval_rows = []
    for lag_k in range(1, MAX_LAG + 1):
        for j in range(1, INTERVALS_PER_DAY + 1):
...
def run_continuous_30min_lags(df, max_interval_lag=360):
    ...
    # Print key lag spikes: k = 16, 32, 48, 64, 80, 160, 320 (multiples of 16 = full day)
    print(f"\n--- Sürekli 30-dk Mum Lag Sıçramaları (Spikes at k = {INTERVALS_PER_DAY}, {INTERVALS_PER_DAY*2}, ...) ---")
    spike_keys = [1, 2, 3, INTERVALS_PER_DAY, INTERVALS_PER_DAY+1,
                  INTERVALS_PER_DAY*2, INTERVALS_PER_DAY*3, INTERVALS_PER_DAY*4,
                  INTERVALS_PER_DAY*5, INTERVALS_PER_DAY*10, INTERVALS_PER_DAY*20]
    spike_keys = [k for k in spike_keys if k <= max_interval_lag]
    spikes = cont_df[cont_df['Interval_Lag_k'].isin(spike_keys)]
...
def run_hourly_periodicity_breakdown(df):
    ...
    k = INTERVALS_PER_DAY  # 1-day lag = 16 tradeable intervals
    hourly_rows = []
...
def plot_interval_heatmap(interval_df):
    ...
    ax.set_yticks(range(INTERVALS_PER_DAY))
...
def plot_continuous_30min_lags(cont_df):
    ...
    # Highlight exact daily multiples (k = 16, 32, 48, 64, 80, ...)
    for d in range(1, 21):
        k_day = d * INTERVALS_PER_DAY
        if k_day <= cont_df['Interval_Lag_k'].max():
            ax.axvline(x=k_day, color='red', alpha=0.25, linestyle='--', linewidth=0.8)

    ax.set_ylabel('Ortalama γ Katsayısı')
    ax.set_title(f'Heston et al. (2010) Tam Yöntemi: Sürekli 30-Dakikalık Bar Lag Yapısı (k = 1 ... {MAX_INTERVAL_LAG} Interval, Müzayede Barları Hariç)')
    ax.legend()
    ax.grid(axis='y', alpha=0.3)
    ...
    ax.set_ylabel('t-İstatistiği')
    ax.set_title(f't-İstatistikleri (Kırmızı Dikey Çizgiler = Tam Gün Katları: {INTERVALS_PER_DAY}, {INTERVALS_PER_DAY*2}, {INTERVALS_PER_DAY*3}... bar gerisi)')
    ax.legend()
    ax.grid(axis='y', alpha=0.3)
...
def plot_hourly_periodicity(hourly_df):
    ...
    ax.set_title('Saat Bazlı Periyodiklik Testi: Trade Edilebilir Saatlerde Periyodiklik (Müzayede Barları Hariç)')
    ax.legend()
    ax.grid(axis='y', alpha=0.3)
...
def generate_report(summary_df, oc_results, oc_interval, cc_results,
    ...
    lines.append(f"- **Veri frekansı:** 30 dakikalık OHLC ({INTERVALS_PER_DAY} trade edilebilir interval/gün, müzayede barları hariç)")
    lines.append(f"- **Hariç tutulan barlar:** 09:30 (açılış müzayedesi), 18:00 (kapanış müzayedesi)")
    ...
    spikes_keys = [1, 2, 3, INTERVALS_PER_DAY, INTERVALS_PER_DAY+1,
                   INTERVALS_PER_DAY*2, INTERVALS_PER_DAY*3, INTERVALS_PER_DAY*4,
                   INTERVALS_PER_DAY*5, INTERVALS_PER_DAY*6, INTERVALS_PER_DAY*7,
                   INTERVALS_PER_DAY*8, INTERVALS_PER_DAY*10, INTERVALS_PER_DAY*20]
    spikes_keys = [k for k in spikes_keys if k <= MAX_INTERVAL_LAG]
    for k_val in spikes_keys:
        r_cont = cont_df[cont_df['Interval_Lag_k'] == k_val]
        if len(r_cont) > 0:
            r_c = r_cont.iloc[0]
            day_str = f"{k_val // INTERVALS_PER_DAY} Gün" if k_val % INTERVALS_PER_DAY == 0 else f"{k_val / INTERVALS_PER_DAY:.2f} Gün"
            spike_str = "🔥 TAM GÜN SPİKE" if k_val % INTERVALS_PER_DAY == 0 else ""
    ...
    lines.append(f"| **Sürekli Lag Yapısı** | k=13, 26, 39, 65'te sıçrama | k={INTERVALS_PER_DAY}, {INTERVALS_PER_DAY*2}, {INTERVALS_PER_DAY*3}, {INTERVALS_PER_DAY*5}, {INTERVALS_PER_DAY*10}'de sıçrama (müzayede barları hariç) | **Birebir Aynı (✓)** |")
    ...
    lines.append(f"| **Gün Sonu Periyodiklik** | 15:30-16:00 arası en yüksek | 17:00-17:30 arası en yüksek (müzayede barları hariç) | **Birebir Aynı (✓)** |")
    lines.append("")
